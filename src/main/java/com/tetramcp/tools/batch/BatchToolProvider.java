...
public class BatchToolProvider extends AbstractToolProvider {
    ...
    private CallToolResult handleBatchDecompile(Program program, List<String> identifiers,
        ...
        // One borrowed interface for the whole batch, released in the finally.
        // This holds a pool slot for the duration of the batch, which is the
        // point: the batch is the workload the pool exists to make cheap.
        ...
        // Every function after a cancellation decompiles to DISPOSED_ON_CANCEL,
        // whose error message is blank, so a batch that reported what it had
        // would render the untried remainder as functions that failed to
        // decompile for no stated reason. Raising is what separates that from a
        // batch in which some functions genuinely did fail.
        if (ProgressReporter.current().isCancelled()) {
            throw new CancellationException("The batch decompile of '" + program.getName()
                + "' was cancelled before every function had been decompiled. The functions "
                + "it did not reach would have been reported as having failed to decompile, "
                + "so no listing is returned. This tool writes nothing, so the program is "
                + "unchanged; there is nothing to undo.");
        }
