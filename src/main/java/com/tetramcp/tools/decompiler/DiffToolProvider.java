...
public class DiffToolProvider extends AbstractToolProvider {
    ...
    protected void defineTools() {
        ...
        addTool(
            Tool.builder().name("functions_tags")
                .description("Manage function tags: list all tags, get tags for a function, " +
                "or add/remove tags from functions.")
                .inputSchema(new JsonSchema("object", Map.of(
                    "action", Map.of("type", "string",
                        "description", "Action: 'list', 'get', 'add', 'remove' (default: 'list')"),
                    "identifier", Map.of("type", "string",
                        "description", "Function name or address (required for get/add/remove)"),
            ...
            (exchange, request) -> {
                ...
                String tag = getOptionalString(request, "tag", null);
                return handleTags(program, action, identifier, tag);
            }
        );
    }
    ...
    private CallToolResult handleTags(Program program, String action,
        ...
        switch (action.toLowerCase()) {
            ...
            case "add": {
                if (identifier == null || tag == null) {
                    throw new IllegalArgumentException(
                        "'identifier' and 'tag' required for 'add' action");
                }
                Function func = resolveFunction(program, identifier);
            ...
            case "remove": {
                if (identifier == null || tag == null) {
                    throw new IllegalArgumentException(
                        "'identifier' and 'tag' required for 'remove' action");
                }
                Function func = resolveFunction(program, identifier);
                com.tetramcp.util.TransactionHelper.executeWriteVoid(
            ...
            default:
                throw new IllegalArgumentException(
                    "Unknown action: '" + action + "'. Use: list, get, add, remove");
        }
    }
