...
public class ProjectToolProvider extends AbstractToolProvider {
    ...
    private CallToolResult handleProjectOpenFile(String path) {
        ...
        try {
            ...
            // getDomainObject makes *this* provider a consumer of the program.
            // ProgramManager takes its own consumer inside openProgram
            // (MultiProgramManager.addProgram -> p.addConsumer(tool)), so ours
            // must be handed back or the program acquires a consumer that is
            // never released. That is not cosmetic: a DomainObject only closes
            // when its last consumer releases it, so a leaked consumer means
            // the user's "close program" never reaches DomainObject.close(),
            // never fires the close listener ProgramRegistry subscribes with,
            // and never releases this program's cached decompilations or its
            // native decompiler subprocesses.
